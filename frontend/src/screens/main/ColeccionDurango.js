...
import ModalAgregarColeccionDurango from '../../components/features/coleccionDurango/ModalAgregarColeccionDurango';
...
export default function ColeccionDurangoScreen() {
  ...
  const fetchItems = useCallback(async (page, searchColumn = null, searchValue = null) => {
    ...
    try {
      ...
      if (searchColumn && searchValue) {
        ...
        url = `/api/coleccionDurango?page=${page}&limit=${itemsPerPage}&column=${encodeURIComponent(searchColumn)}&value=${encodeURIComponent(searchValue)}`;
      } else {
        // Paginación normal
        url = `/api/coleccionDurango?page=${page}&limit=${itemsPerPage}`;
      }
    ...
    if ([10, 20, 30, 40, 50, 60].includes(newItemsPerPage)) {
      ...
      try {
        ...
        if (isSearching && activeSearchColumn && activeSearchValue) {
          ...
          url = `/api/coleccionDurango?page=1&limit=${newItemsPerPage}&column=${encodeURIComponent(activeSearchColumn)}&value=${encodeURIComponent(activeSearchValue)}`;
        } else {
          // Paginación normal con el nuevo límite
          url = `/api/coleccionDurango?page=1&limit=${newItemsPerPage}`;
        }
  ...
  const handleViewDetails = (libroId) => {
    console.log('Opening modal with coleccion Durango ID:', libroId);
  ...
  const handleEditDetails = (libroId) => {
    console.log('Opening edit modal with coleccion Durango ID:', libroId);
  ...
  const handleDeleteDetails = async (libroId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await authenticatedFetch(`${apiHost}/api/coleccionDurango/deactivate/${libroId}`, {
          method: 'PUT',
        });
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <HeaderSon 
        title="Coleccion Durango" 
        searchTerm={searchTerm} 
        setSearchTerm={handleLocalSearch}
        showAdvancedFilter={true}
        columnsEndpoint="/api/coleccionDurango/columns"
