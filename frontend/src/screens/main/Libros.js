...
import ModalAgregarLibros from '../../components/features/Libros/ModalAgregarLibros';
  ...
  const fetchItems = useCallback(async (page, searchColumn = null, searchValue = null) => {
    ...
    try {
      ...
      if (searchColumn && searchValue) {
        ...
        url = `/api/libros?page=${page}&limit=${itemsPerPage}&column=${encodeURIComponent(searchColumn)}&value=${encodeURIComponent(searchValue)}`;
      } else {
        // Paginación normal
    ...
    if ([10, 20, 30, 40, 50, 60].includes(newItemsPerPage)) {
      ...
      try {
        ...
        if (isSearching && activeSearchColumn && activeSearchValue) {
          ...
          url = `/api/libros?page=1&limit=${newItemsPerPage}&column=${encodeURIComponent(activeSearchColumn)}&value=${encodeURIComponent(activeSearchValue)}`;
        } else {
          // Paginación normal con el nuevo límite
          url = `/api/libros?page=1&limit=${newItemsPerPage}`;
        }
  ...
  const handleDeleteDetails = async (libroId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await authenticatedFetch(`${apiHost}/api/libros/deactivate/${libroId}`, {
          method: 'PUT',
        });
