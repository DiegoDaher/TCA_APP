...
import ModalAgregarDiarioOficial from '../../components/features/DiarioOficial/ModalAgregarDiarioOficial';
...
export default function DiarioOficialScreen() {
  ...
  const fetchItems = useCallback(async (page, searchColumn = null, searchValue = null) => {
    ...
    try {
      ...
      if (searchColumn && searchValue) {
        ...
        url = `/api/diario-oficial?page=${page}&limit=${itemsPerPage}&column=${encodeURIComponent(searchColumn)}&value=${encodeURIComponent(searchValue)}`;
      } else {
        // Paginación normal
        url = `/api/diario-oficial?page=${page}&limit=${itemsPerPage}`;
      }
    ...
    if ([10, 20, 30, 40, 50, 60].includes(newItemsPerPage)) {
      ...
      try {
        ...
        if (isSearching && activeSearchColumn && activeSearchValue) {
          ...
          url = `/api/diario-oficial?page=1&limit=${newItemsPerPage}&column=${encodeURIComponent(activeSearchColumn)}&value=${encodeURIComponent(activeSearchValue)}`;
        } else {
          // Paginación normal con el nuevo límite
          url = `/api/diario-oficial?page=1&limit=${newItemsPerPage}`;
        }
  ...
  const handleDeleteDetails = async (libroId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await authenticatedFetch(`${apiHost}/api/diario-oficial/deactivate/${libroId}`, {
          method: 'PUT',
        });
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <HeaderSon 
        ...
        columnsEndpoint="/api/diario-oficial/columns"
        onSearch={handleAdvancedSearch}
        showAddButton={true}
        onAddClick={handleAddClick}
        defaultSearchColumn="Periodo"
      />
