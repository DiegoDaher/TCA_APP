...
const ModalEditarDiarioOficial = ({ isOpen, onClose, libroId }) => {
  ...
  useEffect(() => {
    if (isOpen && libroId) {
      console.log('Fetching diario oficial with ID:', libroId);
      setIsFetching(true);
      setError(null);
      const apiHost = process.env.NEXT_PUBLIC_API_HOST || 'http://localhost:3000';
      
      fetch(`${apiHost}/api/diario-oficial/byId/${libroId}`)
        .then((response) => {
          console.log('Response status:', response.status);
          if (!response.ok) {
            throw new Error(`Error fetching diario oficial data: ${response.statusText}`);
          }
        ...
        .catch((error) => {
          console.error('Error fetching diario oficial data:', error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Actualizando diario oficial con ID:', libroId);
      ...
      const response = await authenticatedFetch(`${apiHost}/api/diario-oficial/${libroId}`, {
        method: 'PUT',
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto m-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-semibold">Editar Diario Oficial</h2>
          </div>
...
export default ModalEditarDiarioOficial;