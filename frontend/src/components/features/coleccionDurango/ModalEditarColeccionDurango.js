...
const ModalEditarColeccionDurango = ({ isOpen, onClose, libroId }) => {
      ...
      fetch(`${apiHost}/api/coleccionDurango/byId/${libroId}`)
        ...
        .then((data) => {
          ...
          if (data && data.data) {
            ...
            setFormData({
              Letra: libroData.Letra || '',
              Titulo: libroData.Titulo || '',
              Autor: libroData.Autor || '',
              Año: libroData.Año || '',
              Editorial: libroData.Editorial || '',
              Edición: libroData.Edición || '',
              ISBN: libroData.ISBN || '',
              Ejemplares: libroData.Ejemplares || ''
            });
          } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Actualizando coleccion durango con ID:', libroId);
      console.log('Cambios detectados:', changes);

      // Asegurarse de que Ejemplares se envía como número (o vacío)
      const payload = {
        ...formData,
        Ejemplares: formData.Ejemplares === '' ? '' : Number(formData.Ejemplares)
      };
      const response = await authenticatedFetch(`${apiHost}/api/coleccionDurango/${libroId}`, {
        method: 'PUT',
  ...
  const handleCancel = () => {
    ...
    setFormData({
      Letra: originalData.Letra || '',
      Titulo: originalData.Titulo || '',
      Autor: originalData.Autor || '',
      Año: originalData.Año || '',
      Editorial: originalData.Editorial || '',
      Edición: originalData.Edición || '',
      ISBN: originalData.ISBN || '',
      Ejemplares: originalData.Ejemplares || ''
    });
    setError(null);
    onClose();
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto m-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-xl font-semibold">Editar Libro - Colección Durango</h2>
          </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={handleInputChange}
                      className="placeholder:text-[var(--selected)]"
                      placeholder="Ej: 978-1-23456-789-9"
                    />
                  </div>
                </div>
...
export default ModalEditarColeccionDurango;
